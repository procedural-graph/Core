...
public abstract class ProxySerializedGraphEntityConverter<TEntity, TModel, TSceneMember, TBaseSceneMember> : 
    GraphNodeSerializer<TBaseSceneMember, TEntity, TModel>, 
    IGraphConverter
    ...
    where TModel : class
{
    ...
    IGraphNode IGraphConverter.ToGraph(object obj, IGraphNode? parent)
    {
        ...
        {
            throw new NotSupportedException($"Proxy entities must be associated with a scene member to be converted to a graph node. Use {nameof(IGraphConverter.ToGraph)} with the model parameter instead.", ex);
        }
    }
